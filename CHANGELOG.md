...
- Cost is shown by the rate it was billed at: uncached input, cache reads, cache writes, and output, with the cache savings stated beside the total. The Overview carries the split under the token strip, and every History conversation carries its own above its sessions.
- Claude Fable 5.1 and Claude Haiku 4.5 sessions are now priced. Rate cards for both were added at OpenRouter's published rates, and a model is matched to its card whichever way its version is spelled — `claude-opus-4-8` finds `claude-opus-4.8`, and a date-stamped id such as `claude-haiku-4-5-20251001` finds the bare card.
- Codex agents are counted as their own sessions under the conversation that spawned them. A spawned agent's rollout is a fork that replays its parent's history before its own turns; the importer used to read the replay as the parent's and either lost the agent or overwrote the parent's turn figures with the fork's mid-turn snapshots. Each rollout is now the session its first metadata line names, replayed turns are skipped without losing the running total, and the parent link is kept on every record. History shows each conversation on one row with its session and agent counts, when it started, when it was last active, how long it ran, and its full tokens and cost; what its agents alone spent sits above the session list, where each agent is nested under its parent with the same dates and duration. A turn whose context Codex re-emits mid-turn, as it does after a context compaction, is now one turn rather than two records fighting over one id. Re-importing a session now replaces its records as a set, so the misattributed rows from earlier imports disappear on the next refresh.
- History lists every conversation in the ledger. It used to receive only the newest 500 usage records, so any conversation older than about a day — every settled T3 thread among them — was silently absent. Conversations are now grouped on the server and sent as groups, which is small enough to send whole.
- History names a Codex or Claude Code session started from T3 by its T3 conversation title, read from T3's local state database on the host where the session ran, in place of the "Codex session 02 · 01a0605d" fallback or Claude Code's own generated title. Only the title and the session it resumes are read, never a thread's messages. A title regenerated in T3 replaces the old one on the next import.
